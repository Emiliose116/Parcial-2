...
def productoMasValioso(
  ...
  // 1. Filtrar productos por condiciones
  val filtrados: List[Producto] = inventario.filter { p =>
    p.precios.length >= minPrecios && p.precios.max > valorBase
  }

  // 2. Mapear cada producto a su promedio (sin tuplas)
  val promedios: List[ProductoPromedio] = filtrados.map { p =>
    val suma = p.precios.foldLeft(0.0)(_ + _)
    val promedio = suma / p.precios.length
    ProductoPromedio(p, promedio)
  }

  // 3. Seleccionar el de mayor promedio de forma manual
  def seleccionarMayor(lista: List[ProductoPromedio]): Option[ProductoPromedio] = {
    lista match {
      case Nil => None
      case cabeza :: cola =>
        var mejor = cabeza
        for (elem <- cola) {
          if (elem.promedio > mejor.promedio) {
            mejor = elem
          }
        }
        Some(mejor)
  ...
  seleccionarMayor(promedios)
}

//Ejemplo de uso
val resultado = productoMasValioso(inventario, valorBase = 20.0, minPrecios = 4)
resultado.foreach { r =>
  println(s"El producto más valioso es: ${r.producto.nombre}, promedio = ${r.promedio}")
}
